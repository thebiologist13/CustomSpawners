...
public abstract class AbstractSpawnManager implements ISpawnManager {
	...
	private Vector randomVector(Vector v1, Vector v2) {
		double x1 = v1.getX();
		double y1 = v1.getY();
		double z1 = v1.getZ();
		double x2 = v2.getX();
		...
		double rX = rand.nextDouble() * Math.abs(x1 - x2);
		double rY = rand.nextDouble() * Math.abs(y1 - y2);
		double rZ = rand.nextDouble() * Math.abs(z1 - z2);
		
		double x3 = Math.min(x1, x2) + rX;
		double y3 = Math.min(y1, y2) + rY;
		double z3 = Math.min(z1, z2) + rZ;
