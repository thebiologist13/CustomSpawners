...
public class EntityInfoCommand extends EntityCommand {
	...
	private String[] getInfo(SpawnableEntity s) {
		...
		for(int i = 0; i < s.getAttributes().size(); i++) {
			Attribute a = s.getAttributes().get(i);
			if(i == 0) {
				attMessage += a.getAttribute().getName() + ", Base=" + a.getBase() + 
						", " + a.getModifiers().size() + " modifiers.";
			} else {
				attMessage += "" + a.getAttribute().getName() + ", Base=" + a.getBase() + 
						", " + a.getModifiers().size() + " modifiers.";
			}
		...
		String[] message = {
				...
				ChatColor.GOLD + "Attributes: " + attMessage,
				ChatColor.GOLD + "Horse Type: " + s.getHorseType().getName(),
				ChatColor.GOLD + "Horse Color: " + s.getHorseColor().getName(),
				ChatColor.GOLD + "Horse Variant: " + s.getHorseVariant().getName(),
				ChatColor.GOLD + "Horse Chests: " + s.hasChests(),
